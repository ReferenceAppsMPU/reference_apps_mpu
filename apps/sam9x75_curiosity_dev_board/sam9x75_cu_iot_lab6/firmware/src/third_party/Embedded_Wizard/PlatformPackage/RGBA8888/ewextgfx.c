...
#include "ew_bsp_display.h"
  ...
  /* variable to store the surface whose CLUT was loaded into graphics accelerator */
...
/* The color format of the framebuffer has to correspond to color format of the
  ...
  #error The given EW_FRAME_BUFFER_COLOR_FORMAT is not supported! Use RGBA8888 within your makefile!
#endif
...
//#if __DCACHE_PRESENT
  ...
  #define EW_CACHE_ALIGNMENT                31
  #define EW_INVALIDATE_CACHE( addr, size ) dcache_InvalidateByAddr( addr, size )
  #define EW_CLEAN_CACHE( addr, size )      dcache_CleanInvalidateByAddr( addr, size )
...
void* GfxCreateSurface( int aFormat, int aWidth, int aHeight )
{
  ...
  pixelBuffer = EwAllocVideo( bitmapSize + clutSize );
  ...
  if ( clutSize > 0 )
    surface->Clut         = (void*)(((unsigned long)pixelBuffer + bitmapSize + 3 ) &~ 3 );
...
#endif
  {
    uint32_t dstColorFormat = GFX_COLOR_MODE_RGBA_8888;
  ...
  int aBlend, unsigned long* aColors )
{
  ...
  /* log the operation */
  #if EW_PRINT_GFX_TASK_DETAILS
    EwPrint( "GfxCopyDriver( 0x%p, 0x%p, ( %d, %d ), ( %d, %d ), ( %d, %d ), %d, "
      "[ #%08X, #%08X, #%08X, #%08X ])\n", aDstHandle, aSrcHandle, aDstX, aDstY,
      aSrcX, aSrcY, aWidth, aHeight, aBlend, aColors[0], aColors[1], aColors[2], aColors[3]);
  #endif
  ...
  {
    uint32_t dstColorFormat = GFX_COLOR_MODE_RGBA_8888;
    uint32_t srcColorFormat = GFX_COLOR_MODE_RGBA_8888;
    uint32_t color          = ( EW_ALPHA( aColors[0]) << 24 )
                            | ( EW_RED  ( aColors[0]) << 16 )
                            | ( EW_GREEN( aColors[0]) <<  8 )
                            |   EW_BLUE ( aColors[0]);
    ...
    if ( srcFormat == EW_PIXEL_FORMAT_ALPHA8 )
      srcColorFormat = GFX_COLOR_MODE_GS_8;
    else if ( srcFormat == EW_PIXEL_FORMAT_RGB565 )
      srcColorFormat = GFX_COLOR_MODE_RGB_565;
    else if ( srcFormat == EW_PIXEL_FORMAT_INDEX8 )
    {
      srcColorFormat = GFX_COLOR_MODE_INDEX_8;

      /* load clut in case of an index8 source bitmap */
      if ( srcSurface != LoadedClutSurface )
      {
        /* clut is loaded only if it differs from previous clut */
  ...
  int aBlend, unsigned long* aColors )
{
  ...
  /* log the operation */
  #if EW_PRINT_GFX_TASK_DETAILS
    EwPrint( "GfxBlendDriver( 0x%p, 0x%p, ( %d, %d ), ( %d, %d ), ( %d, %d ), %d, "
      "[ #%08X, #%08X, #%08X, #%08X ])\n", aDstHandle, aSrcHandle, aDstX, aDstY,
      aSrcX, aSrcY, aWidth, aHeight, aBlend, aColors[0], aColors[1], aColors[2], aColors[3]);
  #endif
