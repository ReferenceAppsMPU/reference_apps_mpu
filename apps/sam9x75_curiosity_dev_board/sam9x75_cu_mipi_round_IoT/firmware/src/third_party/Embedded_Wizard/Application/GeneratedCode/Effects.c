...
void EffectsEffect_run( EffectsEffect _this, XFloat aFrame )
{
      ...
      || ( _this->Timing == EffectsTimingBounce_Out )) || ( _this->Timing == EffectsTimingBounce_InOut )))
  {
    ...
    _this->timingList->spans[ 4 ] = _this->timingList->spans[ 4 ] + ( 1.0f - sum );
  }

  switch ( _this->Timing )
  {
    case EffectsTimingPower_In :
      aFrame = EwMathPow( aFrame, 3.0f );
    break;

    case EffectsTimingPower_Out :
    {
      aFrame = 1.0f - aFrame;
      aFrame = 1.0f - EwMathPow( aFrame, 3.0f );
    }
    break;
    ...
    case EffectsTimingPower_InOut :
    {
      ...
      if ( aFrame < 1.0f )
        aFrame = EwMathPow( aFrame, 3.0f ) / 2.0f;
      else
      {
        aFrame = 2.0f - aFrame;
        aFrame = ( 2.0f - EwMathPow( aFrame, 3.0f )) * 0.5f;
      }
    }
    break;

    case EffectsTimingExp_In :
      aFrame = ( EwMathPow( 2.718f, 3.0f * aFrame ) - 1.0f ) / 19.0793f;
    break;

    case EffectsTimingExp_Out :
    {
      aFrame = 1.0f - aFrame;
      aFrame = 1.0f - (( EwMathPow( 2.718f, 3.0f * aFrame ) - 1.0f ) / 19.0793f );
    }
    break;
    ...
    case EffectsTimingExp_InOut :
    {
      ...
      if ( aFrame < 1.0f )
        aFrame = (( EwMathPow( 2.718f, 3.0f * aFrame ) - 1.0f ) / 19.0793f ) / 2.0f;
      else
      {
        aFrame = 2.0f - aFrame;
        aFrame = ( 2.0f - (( EwMathPow( 2.718f, 3.0f * aFrame ) - 1.0f ) / 19.0793f )) 
        * 0.5f;
      }
    }
    break;
    ...
    case EffectsTimingBack_In :
      aFrame = (( aFrame * aFrame ) * aFrame ) - (( aFrame * 0.5f ) * EwMathSin( 
      aFrame * 180.0f ));
    break;
...
void EffectsOpacityFader__Init( EffectsOpacityFader _this, XObject aLink, XHandle aArg )
{
  ...
  /* Allow the Immediate Garbage Collection to evalute the members of this class. */
  _this->_.XObject._.GCT = EW_CLASS_GCT( EffectsOpacityFader );

  /* ... then construct all embedded objects */
  EffectsInt32Effect__Init( &_this->Effect, &_this->_.XObject, 0 );

  /* Setup the VMT pointer */
  _this->_.VMT = EW_CLASS( EffectsOpacityFader );
}
...
EW_DEFINE_CLASS( EffectsOpacityFader, EffectsFader, Effect, finished, finished, 
                 finished, finished, finished, "Effects::OpacityFader" )
...
void EffectsFadeInOutTransition__Init( EffectsFadeInOutTransition _this, XObject aLink, XHandle aArg )
{
  ...
  /* Allow the Immediate Garbage Collection to evalute the members of this class. */
  _this->_.XObject._.GCT = EW_CLASS_GCT( EffectsFadeInOutTransition );
...
{
  EffectsOpacityFader fader;

  /* Dummy expressions to avoid the 'C' warning 'unused argument'. */
  EW_UNUSED_ARG( _this );

  fader = EwNewObject( EffectsOpacityFader, 0 );
  fader->Super1.Visible = 0;
  fader->Super1.Enabled = 0;
  fader->Super1.RemoveIfHidden = 1;
  fader->Super1.UseCurrentState = 1;
  ...
  EffectsEffect_OnSetCycleDuration((EffectsEffect)&fader->Effect, 500 );
  return ((EffectsFader)fader );
}
...
EW_DEFINE_CLASS( EffectsFadeInOutTransition, EffectsTransition, _.VMT, _.VMT, _.VMT, 
                 _.VMT, _.VMT, _.VMT, "Effects::FadeInOutTransition" )
...
/* Re-Initializer for the auto object 'Effects::FadeInOutCentered' */
void EffectsFadeInOutCentered__ReInit( EffectsFadeInOutTransition _this )
{
  EW_UNUSED_ARG( _this );
}
