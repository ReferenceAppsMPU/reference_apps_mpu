...
#include "ew_bsp_display.h"
  ...
  #error "Double buffering required for SAM9X75!"
#endif
...
volatile int waitingForVSync = 0;
...
int EwBspDisplayInit( int aGuiWidth, int aGuiHeight, XDisplayInfo* aDisplayInfo )
{
  ...
  //LCDC_SetSOFInterruptEnable(true);
...
void EwBspDisplayWaitForCompletion( void )
{
  ...
  while ( waitingForVSync && ( EwGetTicks() - ticks < 100 ))
    ;
...
/* XLCDC Interrupt Handler */
void LCDC_InterruptHandler (void)
{
    // Main ISR - XLCDC
    LCDC_SetSOFInterruptEnable(false);
    register uint32_t status_m = LCDC_REGS->LCDC_LCDISR;
